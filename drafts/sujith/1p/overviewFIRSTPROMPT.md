...
1. Research how to design valid and solvable quantitative word problems so multiple agents can solve them.
2. Generate original MCQ-based quant questions (story-based problems)
3. Self-validate each question using two (or more based on the number of ways to solve it) independent solver agents.
4. Reject and regenerate hallucinated or mathematically invalid questions. This must be done autonomously by an agentic controller/orchestrator.
5. Auto-create a Google Form (or mock quiz) containing the validated questions (in future if workflow is right we will fetch and display in frontend also or store in DB).
6. Produce an accuracy report showing validation performance. Which requires tracking how many questions were generated, how many failed validation, how many regenerations were needed, and final acceptance rate (%). 

Final system should behave like a real autonomous assessment generator with research, generation, validation, and deployment capabilities.

## THE TYPE OF QUESTIONS TO BEGENERATED 
...
These are not easily copy-paste solvable
